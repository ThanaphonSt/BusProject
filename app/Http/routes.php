<?php

/*
|--------------------------------------------------------------------------
| Application Routes
|--------------------------------------------------------------------------
|
| Here is where you can register all of the routes for an application.
| It's a breeze. Simply tell Laravel the URIs it should respond to
| and give it the controller to call when that URI is requested.
|
*/

Route::get('/', 'WelcomeController@index');

Route::get('/home/{id}', 'HomeController@index');

Route::controllers([
	'auth' => 'Auth\AuthController',
	'password' => 'Auth\PasswordController',
]
);

Route::get('/oei', 'HomeController@test');
Route::get('/current', 'BusController@index');
Route::get('/routeofpotong', 'BusController@potongRouteDetail');
Route::get('/routeofsongteaw', 'BusController@songteawRouteDetail');
Route::get('/routeofairportbus', 'BusController@airportBusRouteDetail');
Route::get('/recommend', 'BusController@recommend');
Route::get('/songteawstop', 'BusController@songteawStop');
Route::get('/airportstop', 'BusController@airportStop');
Route::get('/map', 'BusController@map');
Route::get('/routeofpotong1/{id}', 'BusController@map1');
Route::get('/routeofpotong2/{id}', 'BusController@map2');
Route::get('/routeofpotong3/{id}', 'BusController@map3');
Route::get('/recommendshopping', 'BusController@recommendShopping');
Route::get('/recommendhotel', 'BusController@recommendHotel');
Route::get('/recommendrestaurant', 'BusController@recommendRestaurant');
Route::get('/currentbus/{id}', 'BusController@currentbus');